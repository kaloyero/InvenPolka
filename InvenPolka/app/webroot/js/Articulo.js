var Articulo = new Class({
    ...
    bindListEvents:function() {
		...
		jQuery('#artMostrarDisponibles').bind("click", function(e) {
				jQuery("#artMostrarDisponibles").attr("style", "border-color: rgb(105, 131, 236);");	
				jQuery("#artMostrarTodos").attr("style", "");	
				jQuery("#artMostrarConStock").attr("value", "N");
				appStatus.oTable.fnDraw();
		})
		jQuery('#artMostrarTodos').bind("click", function(e) {
//				jQuery("#artMostrarTodos").attr("value", "<b>Mostrar Todos los Articulos</b>");	
//				jQuery("#artMostrarDisponibles").attr("value", "Mostrar Articulos Disponibles");
				jQuery("#artMostrarTodos").attr("style", "border-color: rgb(105, 131, 236);");	
				jQuery("#artMostrarDisponibles").attr("style", "");	
				jQuery("#artMostrarConStock").attr("value", "S");
				appStatus.oTable.fnDraw();
		})
...
//               	return false;
    ...
    afterDataTable:function(data){
		...
		jQuery('#configurationTable_last').bind("click", function(e) {
			self.removerBasuraPluginZoom();
		})
    ...
    drawTableWithThumbnails:function(data){
		...
		mostrarStock = 'S';
		if ( (jQuery("#artMostrarConStock").attr("value") == "S") ){
			mostrarStock = 'S';
		} else {
			mostrarStock = 'N';
		}
	...
	//Preguntamos si estan los botones antes de ponerlos (Por ahi esta conectado un usuario que no le haya venido el boton para usar)
				...
				//Deposito baires
				if (data[i]["_aData"][15] == 'T'){
					htmlDiv +='<div style="position: absolute;margin-top: -44px;margin-left: 95px;"><h4 style="margin-top: -44px;margin-left: 95px;text-align: center;font-size: 14px;color: #568E68;"><b> BAIRES </b></h4></div>'
				}
