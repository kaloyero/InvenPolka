   ...
   public function view($id = null) {
    ...
    public function add() {
        if ($this->request->is('post')) {
			...
			$datasource->begin();
			//sobreescribo el articulo en deposito warnes (id=1)
			$this->request->data['Inventario']['IdDeposito'] = 1;
	...
	function getFieldsToEdit() {
		...
		if(!empty($this->request->data["Articulo"]["idFoto"])){
			...
			$fieldsToEdit= array('CodigoArticulo', 'Descripcion', 'IdCategoria', 'IdObjeto', 'IdEstilo', 'IdMaterial', 'IdDecorado', 'IdDimension','idFoto', 'Disponible', 'DepositoBaires');
		}else{
			$fieldList= array('CodigoArticulo', 'Descripcion', 'IdCategoria', 'IdObjeto', 'IdEstilo', 'IdMaterial', 'IdDecorado', 'IdDimension', 'Disponible', 'DepositoBaires');
		}
			return $fieldList;
	}
