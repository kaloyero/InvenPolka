...
\********************************************************************************/
	...
	function getArticulosVistaByArrayId($ids) {
		...
		//Borro la ultima coma
		$inCondition = substr_replace( $inCondition, "", -1 );
		$articulos =$articulo->query("Select *, IF(  `stock_dispo` >0,  '1',  '0' ) AS stockOrder from `articulos_vista`  WHERE `id` IN (".$inCondition.") ORDER BY stockOrder desc , Disponible desc;");
		return $articulos;
	}
